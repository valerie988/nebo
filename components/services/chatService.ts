...
export const chatService = {
  ...
  async sendMessage(params: {
    ...
    await chatService._updatePreview(params.conversationId, msg.text, msg.createdAt, false);
    const pending = await read<Message[]>(KEYS.pending, []);
    await write(KEYS.pending, [...pending, msg]);
    return msg;
  },

  async confirmMessage(localId: string, serverId: string, conversationId: string): Promise<void> {
    const msgs = await read<Message[]>(KEYS.messages(conversationId), []);
  ...
  async receiveMessage(msg: Message): Promise<void> {
    ...
    await chatService._updatePreview(msg.conversationId, msg.text, msg.createdAt, true);
  },
  ...
  async _updatePreview(id: string, text: string, at: string, inc: boolean): Promise<void> {
    const all = await read<Conversation[]>(KEYS.conversations, []);
    await write(KEYS.conversations, all.map(c => c.id !== id ? c : {
